...
export class DeliveryMenService {
  ...
  async getOrdersByDeliveryMen(req: any) {
    ...
    const query = this.orderRepository
      ...
      .leftJoinAndSelect('order.userData', 'user_data')
      .where(
        'order.delivery_men = :deliveryMenId AND order.user_order_status = :status',
        { deliveryMenId, status: 7 },
      );
      ...
      const result = await Promise.all(
        data.map((order) =>
          limit(async () => {
            ...
            const [user, pharmacy, pharmacieslocation, location, distanceInfo] =
              await Promise.all([
                this.getUserDetails(order.user_id, 4),
                this.getUserDetails(pharmacies.pharmacy.id, 3),
                this.baseService.getLocationFromLatLong(
                  pharmacies.pharmacy.latitude,
                  pharmacies.pharmacy.longitude,
                ),
                this.baseService.getLocationFromLatLong(latitude, longitude),
  ...
  async getOrdersDetails(orderId: number, req: any) {
    try {
      ...
      order['deliveryMen_latitude'] = order.deliveryMen?.latitude
        ...
        : null;
      order['deliveryMen_longitude'] = order.deliveryMen?.longitude
        ? parseFloat(order.deliveryMen.longitude)
        : null;
    ...
    try {
      ...
      if (!frontFile) {
        throw new BadRequestException(
          'Please provide both front and back ID photos with correct field names',
        );
      }
      ...
      throw new InternalServerErrorException(
        'Error saving files data: ' + error.message,
      );
    }
  }
  ...
  async addAccount(addAccountDto: AddAcountDto, req: any) {
    const { id } = req.user;
    const { account_number, pay_system, pay_type, IFSC, customer_id, number } =
      addAccountDto;
    if (pay_type === 'bankaccount') {
      if (!account_number || !IFSC || !customer_id) {
        throw new BadRequestException(
          'Please provide Account Number, IFSC and Customer ID',
        );
      }
  ...
  async getAccounts(req: any) {
    try {
      const { id } = req.user;
      const records = await this.userPayDataRepository.find({
        where: { user_id: id },
      });
  ...
  async updateAccountPriority(req: any, body: any) {
    try {
      ...
      await this.userPayDataRepository.update(
        { user_id: id, pay_type },
        { priority: priority },
      );
      return {
  ...
  async deleteAccount(req: any, body: any) {
    try {
      ...
      if (pay_type === 'bankaccount') {
        ...
        await this.userPayDataRepository.delete({
          user_id: id,
          pay_type,
          field_key: 'account_number',
          field_value: account_number,
  ...
  async getFaq(req: any, lang: any) {
    ...
    const data = await this.faqRepository.find({
      where: { lang: lang.lang, role_id: role_id },
      select: ['question', 'answer', 'lang', 'sorting'],
    });
    if (!data) {
  ...
  async updateOnline(isOnline: { isOnline: number }, req) {
    try {
      ...
      await this.userRepository.update({ id }, { online: isOnline.isOnline });
